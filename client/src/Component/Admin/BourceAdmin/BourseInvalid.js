                ...
                <div className='flex flex-wrap'>
                    {bourses.map((bourse, index) => (
                        <div key={index} className={`p-3 rounded-xl h-max ${mobile ? 'w-[98%]' : 'w-[15em] '}  m-2`}>
                            ...
                            <Link to={`/bourse/archive/detail/${encryptData((bourse.id).toString())}`} className='w-full block hover:text-white focus:text-white text-center cursor-pointer py-1.5 mt-1 bg-fuchsia-600 text-white rounded'>
                                Detail
                            </Link>
                        </div>
                    ))}

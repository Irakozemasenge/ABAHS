...
const EditorAdminEvent = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!photoOrder && !video && !photo) {
            toast.error("Veuillez ajouter soit une vidéo soit une photo.");
            return;
        }

        if (video && photo) {
            toast.error("Vous ne pouvez pas ajouter à la fois une vidéo et une photo.");
            return;
        }
            ...
            <div className='w-full overflow-y-auto overflow-x-hidden h-[80vh]'>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col items-center mb-5">
                    <div className="rounded-lg p-2 border border-blue-500 w-[90%] sm:p-4">
                        <div className="justify_center rounded-xl w-full p-1 sm:p-4 mb-10 border border-fuchsia-700 flex flex-col">
                            ...
                            <label htmlFor={`photo`} className={`appearance-none outline-none focus:border focus:border-fuchsia-600 block cursor-pointer bg-transparent w-full  text-grey-darker border border-red rounded py-3 px-4 mb-3 `}>
                                photo
                            </label>
                            <input
